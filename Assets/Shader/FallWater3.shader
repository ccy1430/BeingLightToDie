Shader "Unlit/FallWater"
{
	Properties
	{
		...
		_XScale("XScale",Float) = 2
		_YScale("YScale",Float) = 0.2
	...
	SubShader
	{
		...
		Pass
		{
			...
			float perlinNoise(float2 uv)
			{
				...
				float2 wp = pow(pf,2)*(30*pf*pf-60*pf+30);
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				col.a *= dis;
				col.a*=uvnoise;
				col.a = 4*col.a*(1-col.a)+col.a*col.a;
				//col.a = 2*pow(col.a,3)+3*pow(col.a,2);
				if(i.uv.y<0.1){
					col.rgb = lerp(_DeepColor.rgb,col.rgb,10*i.uv.y);
				}
				if(i.uv.x<0.1||i.uv.x>0.9){
					float val = 10*(abs(i.uv.x-0.5)-0.4);
					col.a = lerp(col.a,0,val);
				}
				return col;
			}
			ENDCG
